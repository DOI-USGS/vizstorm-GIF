prep_outro_rdgs_fun <- function(rdg_ind="1_fetch/out/rapid_dep_sites.rds.ind", gage_col_config, outro_placement, legend_text_cfg, opacity=1) {
  ...
  plot_fun <- function(){
    ...
    x_title <- user_coords[1] + 0.57 * diff(user_coords[1:2])
    x_text <- user_coords[1] + 0.57 * diff(user_coords[1:2])
    y_title <- user_coords[3] + 0.33 * diff(user_coords[3:4])
    y_text <- user_coords[3] + 0.29 * diff(user_coords[3:4])
    x_dot <- x_text + 0.297 * diff(user_coords[1:2])
    y_dot <- y_text - 0.038 * diff(user_coords[3:4])
    ...
    text_chars <- "During floods, USGS rapidly deploys
extra gages (orange dots   ) to
...
prep_outro_allsites_fun <- function(allsites_ind="2_process/out/gage_sites_geom.rds.ind",
  ...
  plot_fun <- function(){
    ...
    # plan text and legend coordinates
    user_coords <- par()$usr
    x_title <- user_coords[1] + 0.67 * diff(user_coords[1:2])
    x_text <- user_coords[1] + 0.57 * diff(user_coords[1:2])
    y_title <- user_coords[3] + 0.58 * diff(user_coords[3:4])
    y_text <- user_coords[3] + 0.51 * diff(user_coords[3:4])
    x_dot <- x_text + 0.128 * diff(user_coords[1:2])
    y_dot <- y_text - 0.067 * diff(user_coords[3:4])
    # plot text and legend
    text(x=x_title, y=y_title, labels="        NATIONAL SCALE\nOBSERVING NETWORK", adj=c(0, 1),
         cex=2.2, col=legend_text_cfg$col, family = 'Oswald')
    text_chars <- "           USGS monitors water levels
at thousands of gages nationally
(blue dots   ). Long-term USGS gages
across the southeastern US are
shown on the map."
    text(x=x_text, y=y_text, labels=text_chars, adj=c(0, 1),
         cex=1.8, col=legend_text_cfg$col, family = 'abel')
...
prep_outro_readmore_fun <- function(outro_placement, legend_text_cfg, opacity=1) {
  ...
  plot_fun <- function(){
    ...
    x_title <- user_coords[1] + 0.57 * diff(user_coords[1:2])
    x_text <- user_coords[1] + 0.57 * diff(user_coords[1:2])
    y_title <- user_coords[3] + 0.15 * diff(user_coords[3:4])
    y_text <- user_coords[3] + 0.11 * diff(user_coords[3:4])

    # plot text and legend
    text(x=x_title, y=y_title, labels="STAY SAFE DURING FLOODS", adj=c(0, 1),
         cex=2.2, col=legend_text_cfg$col, family = 'Oswald')
    text_chars <- "Learn more about USGS response to Michael\nat www.usgs.gov/michael"
    text(x=x_text, y=y_text, labels=text_chars, adj=c(0, 1),
