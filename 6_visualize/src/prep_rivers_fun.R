...
prep_rivers_fun <- function(river_ind, rivers_cfg){
  ...
  plot_fun <- function(){
    plot(st_geometry(sf_rivers$sf_gage_rivers), col = rivers_cfg$gage_river_col, lwd = .5, add = TRUE)
    plot(st_geometry(sf_rivers$sf_major_rivers), col = rivers_cfg$major_river_col, lwd = 1, add = TRUE)
