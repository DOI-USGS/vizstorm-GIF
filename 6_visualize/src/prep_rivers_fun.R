...
prep_rivers_fun <- function(river_ind, rivers_cfg){
  ...
  # the great lakes look goofy (ocean + lake coloring), so remove them. this
  # really should happen in process rather than visualize
  sf_rivers$sf_waterbodies <- sf_rivers$sf_waterbodies %>%
    filter(!gnis_name %in% c('Lake Erie','Lake Huron','Lake Michigan','Lake Ontario'))
