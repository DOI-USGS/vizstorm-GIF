prep_gage_sites_fun <- function(stage_data, gage_col_config, DateTime){
  ...
  below_freezing_sites <- filter(this_stage_data, temp_f <= 32)
  ...
  plot_fun <- function(){
    plot(sf::st_geometry(below_freezing_sites$geometry),
         pch = 18, add = TRUE,
         bg = gage_col_config$gage_below_freezing_col, cex = 5,
         col = gage_col_config$gage_below_freezing_border)
